...
async def button_click(update: Update, context: CallbackContext):
    ...
    if data[0] == "retryCheckCash":
        ...
        if response.status_code== 200:
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id,text="البوت يواجه مشكلة في التحقق، الرجاء المحاولة في وقتٍ لاحق")
...
""",
        ...
        if response.status_code == 200:
            ...
            if response_msg['status']=="success":
                ...
                await context.bot.send_message(chat_id=update.effective_chat.id,text="أدخل اسم المستخدم",reply_markup=create_keyboard(keyboard))
...
async def fifth_question(update: Update, context: CallbackContext) -> int:
    ...
    if float(my_balance) < float(response_to_question5):
        await context.bot.send_message(chat_id=update.effective_chat.id,text="🚫 لايوجد رصيد كافي في حسابك لشحن المبلغ المطلوب \n\n أدخل مبلغ شحن بكافئ رصيدك الحالي في البوت أو دون:",reply_markup=create_keyboard(keyboard), parse_mode='HTML')
        return FIFTH_QUESTION
    else:
...
async def sixth_question(update: Update, context: CallbackContext) -> int:
    ...
    if result["status"] == "InsuffBalance":
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id,text="أدخل مبلغ يكافئ رصيدك في حساب أيشانسي:",reply_markup=create_keyboard(keyboard))
        return SIXTH_QUESTION
    else:    
        await context.bot.send_message(chat_id=update.effective_chat.id,text=result["message"],reply_markup=create_keyboard(keyboard))
        return ConversationHandler.END
